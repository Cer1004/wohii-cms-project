...
async function loadQuestions(keyword = "", page = 1) {
  ...
  try {
        ...
        <article class="question-card ${q[CONFIG.API_FIELDS.SOLVED] ? "solved-card" : ""}">
          ...
          <div class="question-actions">
            <span>
              ...
              <button class = "btn btn-like" data-id="${q.id}">😍 ${q.likeCount || 0} </button>
            </span>
            ${
    ...
    const isLiked = questions.find(q => q.id == el.dataset.id).liked;
    toggleLike(el.dataset.id, isLiked);
  });
});
...
async function toggleLike(questionId, isLiked) {
  try {
    await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${questionId}/like`, {
