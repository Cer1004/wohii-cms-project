...
describe("POST /api/questions/:questionId/like", () => {
  it("returns 404 when liking an unknown question", async () => {
    const token = await registerAndLogin();
    const res = await request(app)
      .post("/api/questions/99999/like")
  ...
  it("returns 201 with liked=true and likeCount=1 on first like", async () => {
    ...
    const res = await request(app)
      .post(`/api/questions/${post.id}/like`)
      .set("Authorization", `Bearer ${token}`);
